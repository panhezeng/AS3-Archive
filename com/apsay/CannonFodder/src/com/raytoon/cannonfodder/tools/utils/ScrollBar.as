package com.raytoon.cannonfodder.tools.utils
{
	...
	public class ScrollBar
	{
		...
		public function set tween(p_value:Number):void
		{
			_tween = p_value < 1 || p_value > 20 ? 1 : p_value;
		}

		public function set elastic(p_value:Boolean):void
		{
			_elastic = p_value ;
			if(_abled)
				makeScrollBar();
		...
		public function set lineAbleClick(p_value:Boolean):void
		{
			_lineAbleClick = p_value ;
			if(_lineAbleClick)
				_scrollLine.addEventListener(MouseEvent.MOUSE_DOWN, scrollLineMouseDownHandler);
		...
		public function set mouseWheel(p_value:Boolean):void
		{
			_mouseWheel = p_value ;
			if(_mouseWheel && _abled)
				_stage.addEventListener(MouseEvent.MOUSE_WHEEL, mouseWheelHandler);
			// _stage.addEventListener(MouseEvent.MOUSE_WHEEL, mouseWheelHandler, false, 0, true);
		...
		public function set direction(p_value:String):void
		{
			_direction = p_value ;
		...
		public function set scale9Grid(p_value:Rectangle):void
		{
			_scale9Grid = p_value ;
			try
			{
		...
		{
			_speed = p_value < 5 || p_value > 35 ? 15 : p_value;
		}

		public function set UP(p_target:Sprite):void
		{
			_upBtn = p_target;
			if(!_abled)
			{
			...
			_upBtn.addEventListener(MouseEvent.MOUSE_DOWN, upDownBtnMouseDownHandler);
			// _upBtn.addEventListener(MouseEvent.MOUSE_DOWN, upDownBtnMouseDownHandler, false, 0, true);
		}

		public function set DOWN(p_target:Sprite):void
		{
			_downBtn = p_target;
			if(!_abled)
			{
			...
			_downBtn.addEventListener(MouseEvent.MOUSE_DOWN, upDownBtnMouseDownHandler);
			// _downBtn.addEventListener(MouseEvent.MOUSE_DOWN, upDownBtnMouseDownHandler, false, 0, true);
		}

		/**
		 * 双向(横板+竖版)滚动条
		 * @param		p_target		      	DisplayObjectContainer		 * 被遮罩对象
		 * @param		p_maskTarget		    *		        			 * 遮罩对象(可传入Rectangle类型)
		 * @param		p_scrollBar		        Sprite		     		     * 滚动滑块
		 * @param		p_scrollLine		    Sprite		      			 * 滚动条
		 * @param		p_tween		            Number		         		 * 缓动系数
		 * @param		p_elastic		        Boolean		       			 * 滑块可否拉伸
		 * @param		p_lineAbleClick		    Boolean		      	         * 滚动条可否点击
		 * @param		p_mouseWheel		   	Boolean		      	         * 滚轮可用
		 * @param		p_direction		    	String		      	         * 方向(默认纵向)
		 */
		public function ScrollBar(p_stage:Stage, p_target:DisplayObjectContainer, p_maskTarget:*, p_scrollBar:Sprite, p_scrollLine:Sprite, p_tween:Number = 0, p_elastic:Boolean = true, p_lineAbleClick:Boolean = false, p_mouseWheel:Boolean = true, p_direction:String = "L")
		{
			if(!((p_maskTarget is DisplayObject) || (p_maskTarget is Rectangle))) throw(new Error("没有传入遮罩对象"));

			_target = p_target;
			_maskTarget = p_maskTarget is Rectangle ? drawMaskTarget(p_maskTarget) : p_maskTarget;
			_scrollBar = p_scrollBar;
			_scrollLine = p_scrollLine;
			_tween = p_tween < 1 || p_tween > 20 ? 1 : p_tween;
			_elastic = p_elastic;
			_lineAbleClick = p_lineAbleClick;
			_mouseWheel = p_mouseWheel;
			_direction = p_direction;
			_stage = p_stage;
			_parentMC = _scrollBar.parent;
			_coor = p_direction == "H" ? "x" : "y";
			_length = _coor == "x" ? "width" : "height";
			_mouse = _coor == "x" ? "mouseX" : "mouseY";
		...
		private function makeScrollBar():void
		{
			...
			_stage.addEventListener(MouseEvent.MOUSE_UP, scrollBarMouseUpHandler);
			// _stage.addEventListener(MouseEvent.MOUSE_UP, scrollBarMouseUpHandler, false, 0, true);
			// _scrollBar.parent.stage.addEventListener(MouseEvent.MOUSE_UP, scrollBarMouseUpHandler, false, 0, true);
			_stage.addEventListener(Event.MOUSE_LEAVE, scrollBarMouseUpHandler);
			// _stage.addEventListener(Event.MOUSE_LEAVE, scrollBarMouseUpHandler, false, 0, true);
		}
		...
		private function scrollBarMouseUpHandler(e:Event):void
		{
		...
		// 按住内容容器后，移动鼠标可模拟向上向下拖动内容
		public function mouseMove(upward:Boolean, moveSpeed:Number):void
		{
			if(upward)
			{
				_scrollBar[_coor] += moveSpeed;
			}
			else
			{
				_scrollBar[_coor] -= moveSpeed;
		...
		// 绘制遮罩
		private function drawMaskTarget(p_rect:Rectangle):Sprite
		{
			var maskTarget:Sprite = new Sprite();
			maskTarget.graphics.beginFill(0xffffff);
			maskTarget.graphics.drawRect(p_rect.x, p_rect.y, p_rect.width, p_rect.height);
			maskTarget.graphics.endFill();
			return maskTarget;
		}

		// 注册点移到左上角
		private function setRegistration(p_target:DisplayObjectContainer):void
		{
			var rect:Rectangle = p_target.getRect(p_target);
			var _x:Number = rect.x;
			var _y:Number = rect.y;
			var depth:Number = p_target.numChildren;
			if(depth == 0) return;
			for(var i:uint = 0; i < depth; i++ )
			{
				var target:DisplayObject = p_target.getChildAt(i);
				target.x -= _x;
				target.y -= _y;
			}
		...
		public function destroy():void
		{
			if(_timer != null)
			{
				if(_timer.hasEventListener(TimerEvent.TIMER)) _timer.removeEventListener(TimerEvent.TIMER, timeHandler);
				_timer = null;
			}
			if(_scrollBar.hasEventListener(Event.ENTER_FRAME)) _scrollBar.removeEventListener(Event.ENTER_FRAME, scrolBarEnterFrameHandler);
			if(_scrollBar.hasEventListener(MouseEvent.MOUSE_DOWN)) _scrollBar.addEventListener(MouseEvent.MOUSE_DOWN, scrollBarMouseDownHandler);
			if(_scrollBar.hasEventListener(MouseEvent.MOUSE_UP)) _scrollBar.addEventListener(MouseEvent.MOUSE_UP, scrollBarMouseUpHandler);
			if(_scrollLine.hasEventListener(MouseEvent.MOUSE_DOWN)) _scrollLine.removeEventListener(MouseEvent.MOUSE_DOWN, scrollLineMouseDownHandler);
			if(_stage.hasEventListener(MouseEvent.MOUSE_WHEEL)) _stage.removeEventListener(MouseEvent.MOUSE_WHEEL, mouseWheelHandler);
			if(_stage.hasEventListener(MouseEvent.MOUSE_UP)) _stage.addEventListener(MouseEvent.MOUSE_UP, scrollBarMouseUpHandler);
			if(_stage.hasEventListener(Event.MOUSE_LEAVE)) _stage.addEventListener(Event.MOUSE_LEAVE, scrollBarMouseUpHandler);
			if(_upBtn != null)
			{
				if(_upBtn.hasEventListener(MouseEvent.MOUSE_DOWN)) _upBtn.removeEventListener(MouseEvent.MOUSE_DOWN, upDownBtnMouseDownHandler);
			}
			if(_downBtn != null)
			{
				if(_downBtn.hasEventListener(MouseEvent.MOUSE_DOWN)) _downBtn.removeEventListener(MouseEvent.MOUSE_DOWN, upDownBtnMouseDownHandler);
			}
